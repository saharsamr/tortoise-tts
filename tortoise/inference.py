...
if __name__ == '__main__':
    ...
    2- fast: the second fastest option, and the quality is better than the first option. it is recommended for mass generation.
    ...
    audio will be made of). as the model can mess up in some parts, you can specify which clips you want to re-generate
    by passing the clip numbers to --regenerate argument. clip numbers should be separated by commas.
    """
    ...
    for selected_voice in selected_voices:
        """
        the path to save each final audio is sepcified in voice_outpath variable, using the outpath, and the voice name.
        then if the directory doesn't exist, it will be created.
        """
        ...
        for j, text in enumerate(texts):
            """
            if we are running this script to regenerate the parts that was messed up in previous renders, the regererate
            variable will be a list of messed up segment numbers. if so, there is no need to regenerate the sections 
            which are not listed after the --regenerate argument. so the following if checks whether we want to do
            regeneration, the if we are in a good section (which is not listed in input arguments) it simply loads the
            previously-generated audio to be used in this round, then it goes for the next part of text in texts 
            variable. if we are regenerating, and j IS in regenerate, the audio will be again produced.
            """
